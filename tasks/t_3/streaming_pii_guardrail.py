...
class PresidioStreamingPIIGuardrail:
    ...
    def process_chunk(self, chunk: str) -> str:
        ...
        if len(self.buffer) > self.buffer_size:
            ...
            anonymized_text = self.anonymizer.anonymize(text=text_to_process, analyzer_results=results)
    ...
    def finalize(self) -> str:
        if self.buffer:
            ...
            anonymized_text = self.anonymizer.anonymize(text=self.buffer, analyzer_results=results)
            self.buffer = ""
            return anonymized_text.text
        return ""
...
def main():
    ...
    while True:
        ...
        response_chunks = llm_client.stream(messages)

        final_response = ""
        for chunk in response_chunks:
            safe_chunk = guardrail.process_chunk(chunk.content)
            if safe_chunk:
                print(f"Assistant (streaming): {safe_chunk}", end='', flush=True)
                final_response += safe_chunk

        # Finalize to process any remaining buffer content
        final_safe_chunk = guardrail.finalize()
        if final_safe_chunk:
            print(f"Assistant (final): {final_safe_chunk}", end='', flush=True)
            final_response += final_safe_chunk
